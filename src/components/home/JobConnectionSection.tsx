...
const JobConnectionSection = () => {
  ...
  const renderJobsContent = () => <>
      <div className="grid lg:grid-cols-2 gap-6 items-start">
        ...
        <div className="bg-card border border-border rounded-2xl overflow-hidden shadow-lg flex flex-col lg:flex-row h-auto lg:h-[280px]">
          ...
          <div className="lg:w-1/2 p-6 flex flex-col justify-center">
            ...
            <div className="flex flex-col gap-3">
              {locations.map(loc => <button key={loc.key} onClick={() => handleLocationChange(loc.key)} className={`flex items-center gap-2 px-4 py-3 rounded-lg border-2 transition-all duration-300 text-left ${selectedLocation === loc.key ? "border-primary bg-primary/10 text-primary shadow-md" : "border-border bg-muted/30 text-foreground hover:border-primary/50"}`}>
        ...
        {/* Right: Job Details */}
        {selectedCompany && <div className="bg-card border border-border rounded-2xl overflow-hidden shadow-lg hover:shadow-xl transition-shadow duration-300 flex flex-col lg:flex-row h-auto lg:h-[280px]">
            <div className="lg:w-1/2 h-48 lg:h-full flex flex-col">
              <div className="relative overflow-hidden h-2/3">
                {selectedCompany.logo ? <img src={selectedCompany.logo} alt={selectedCompany.name} className="w-full h-full object-cover hover:scale-105 transition-transform duration-700" /> : <div className="w-full h-full bg-primary/10 flex items-center justify-center">
                ...
                <div className="flex items-start gap-2">
                  ...
                  <div>
                    ...
                    <div className="flex flex-wrap gap-1 mt-1">
                      {selectedCompany.benefits.map((benefit, idx) => <span key={idx} className="text-xs bg-primary/10 text-primary px-2 py-0.5 rounded-full">{benefit}</span>)}
                    </div>
                  </div>
                </div>
              </div>
      ...
      <div className="mt-8">
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3">
          {companies.map((company, index) => <button key={company.id} onClick={() => setSelectedCompanyIndex(index)} className={`p-3 rounded-xl border-2 transition-all duration-300 text-left ${selectedCompanyIndex === index ? "border-primary bg-primary/10 shadow-md" : "border-border bg-card hover:border-primary/50 hover:shadow-sm"}`}>
              <div className="w-12 h-12 rounded-lg overflow-hidden mx-auto mb-2">
                {company.logo ? <img src={company.logo} alt={company.name} className="w-full h-full object-cover" /> : <div className="w-full h-full bg-muted flex items-center justify-center">
  ...
  const renderCandidatesContent = () => <div className="grid lg:grid-cols-2 gap-6">
      {candidates.map(candidate => <div key={candidate.id} className="bg-card border border-border rounded-2xl overflow-hidden shadow-lg hover:shadow-xl transition-shadow duration-300 flex flex-col lg:flex-row h-auto lg:h-[280px]">
          <div className="lg:w-1/2 h-48 lg:h-full flex flex-col">
            <div className="relative overflow-hidden h-2/3">
  ...
  const renderInternshipContent = () => <div className="grid lg:grid-cols-2 gap-6 items-start">
      {/* Left: Info Card */}
      <div className="bg-card border border-border rounded-2xl overflow-hidden shadow-lg flex flex-col lg:flex-row h-auto lg:h-[280px]">
        <div className="lg:w-1/2 h-48 lg:h-full flex flex-col">
          <div className="relative overflow-hidden h-2/3">
          ...
          <div className="space-y-2 flex-1 overflow-hidden">
            ...
            <div className="flex items-start gap-2">
              ...
              <div>
                <p className="text-xs text-muted-foreground">Vị trí</p>
                <div className="flex flex-wrap gap-1 mt-1">
                  {internshipPrograms[0].positions.map((pos, idx) => <span key={idx} className="text-xs bg-primary/10 text-primary px-2 py-0.5 rounded-full">{pos}</span>)}
                </div>
              </div>
            </div>
          </div>
  ...
  return <section className="utt-section bg-gradient-to-br from-background via-background to-accent/20 relative overflow-hidden">
      ...
      <div className="w-full max-w-[1800px] mx-auto px-4 lg:px-8 relative z-10">
        ...
        <div className="flex flex-wrap justify-center gap-3 mb-12">
          {tabButtons.map(tab => <button key={tab.key} onClick={() => setActiveTab(tab.key)} className={`group flex items-center gap-3 px-6 py-4 rounded-2xl border-2 transition-all duration-300 ${activeTab === tab.key ? "border-primary bg-primary text-primary-foreground shadow-lg" : "border-border bg-card text-foreground hover:border-primary/50 hover:shadow-md"}`}>
              <div className={`w-10 h-10 rounded-xl flex items-center justify-center transition-colors ${activeTab === tab.key ? "bg-white/20" : "bg-primary/10"}`}>
