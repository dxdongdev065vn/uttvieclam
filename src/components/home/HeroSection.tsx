...
const HeroSection = () => {
  ...
  return <section className="relative w-full h-[400px] md:h-[500px] lg:h-[550px]">
      ...
      <div className="absolute bottom-4 left-1/2 lg:left-1/3 -translate-x-1/2 flex gap-2 z-20">
        {slides.map((_, index) => <button key={index} onClick={() => goToSlide(index)} className={`w-2.5 h-2.5 rounded-full transition-colors ${index === currentSlide ? "bg-primary" : "bg-background/60 hover:bg-background/80"}`} />)}
      </div>
        ...
        <div className="bg-background/95 backdrop-blur-sm rounded-2xl shadow-2xl border border-border/50 p-6 lg:p-8 w-full max-w-md lg:max-w-none lg:mx-6 lg:my-8">
          ...
          <div className="space-y-4">
            <div className="relative">
              ...
              <Input type="text" placeholder="Tìm việc làm, công ty, vị trí..." value={searchQuery} onChange={e => setSearchQuery(e.target.value)} className="pl-12 pr-4 py-6 text-base rounded-lg border-border" />
            </div>
          ...
          <div className="mt-6">
            ...
            <div className="flex flex-wrap gap-2">
              {quickSearchTags.map(tag => <button key={tag} className="px-4 py-1.5 text-sm border border-border rounded-full hover:border-primary hover:text-primary transition-colors bg-background">
                  {tag}
                </button>)}
            </div>
          </div>
