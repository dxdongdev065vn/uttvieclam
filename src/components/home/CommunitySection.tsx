...
import { Users, Heart, Award, UserCheck, Handshake, GraduationCap, Lightbulb, Calendar, Trophy, ArrowRight, Briefcase, MapPin, Star, Code, Rocket, DollarSign, Clock, Target, Building } from "lucide-react";
...
const CommunitySection = () => {
  return <section className="py-16 md:py-20 bg-gradient-to-b from-muted/30 to-background relative overflow-hidden">
      {/* Decorative elements */}
      ...
      <div className="w-full max-w-[1800px] mx-auto px-4 lg:px-8 relative z-10">
        ...
        <div className="text-center mb-12">
          <div className="inline-flex items-center gap-2 bg-primary/10 border-primary/20 rounded-full px-6 py-2.5 mb-4 border">
            <Users className="w-5 h-5 text-primary" />
            <span className="text-base font-semibold text-primary">Kết Nối Hệ Sinh Thái</span>
          </div>
        ...
        <div className="grid md:grid-cols-3 gap-6 lg:gap-8">
          ...
          <div className="bg-card rounded-2xl border border-border shadow-lg overflow-hidden flex flex-col">
            ...
            <div className="p-4 flex-1 space-y-3">
              {alumniList.map(alumni => <Link key={alumni.id} to={`/cong-dong/cuu-sv/${alumni.id}`} className="block bg-muted/50 hover:bg-muted rounded-xl p-3 transition-all hover:shadow-md group">
                  <div className="flex items-start gap-3">
                    <img src={alumni.avatar} alt={alumni.name} className="w-12 h-12 rounded-full object-cover border-2 border-blue-200" />
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 mb-1">
                        ...
                        {alumni.isMentor && <Badge className="text-xs bg-green-500">Mentor</Badge>}
                      </div>
          ...
          <div className="bg-card rounded-2xl border border-border shadow-lg overflow-hidden flex flex-col">
            ...
            {/* Content - Clubs List */}
            <div className="p-4 flex-1 space-y-3">
              {clubsList.map(club => <Link key={club.id} to={`/cong-dong/clb/${club.id}`} className="block bg-muted/50 hover:bg-muted rounded-xl p-3 transition-all hover:shadow-md group">
                  <div className="flex items-center gap-2 mb-2">
                    <Badge className="bg-rose-500">
                      <Heart className="w-3 h-3 mr-1" />
                      CLB
                    </Badge>
                    {club.isActive && <Badge variant="outline" className="text-xs text-green-600 border-green-300">
                        Đang hoạt động
                      </Badge>}
                  </div>
          ...
          <div className="bg-card rounded-2xl border border-border shadow-lg overflow-hidden flex flex-col">
            ...
            <div className="p-4 flex-1 space-y-3">
              {scholarshipsAndFunds.map(item => <Link key={item.id} to={`/cong-dong/hoc-bong/${item.id}`} className="block bg-muted/50 hover:bg-muted rounded-xl p-3 transition-all hover:shadow-md group">
                  <div className="flex items-center gap-2 mb-2">
